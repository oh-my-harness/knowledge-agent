...
#[tokio::test]
async fn vault_pdfs_returns_pdf_assets() {
    let vault = tempfile::tempdir().expect("tempdir");
    let papers = vault.path().join("assets").join("papers");
    std::fs::create_dir_all(&papers).unwrap();
    std::fs::write(papers.join("source.pdf"), b"%PDF-1.4\n").unwrap();
    ...
    assert_eq!(json[0]["path"], "assets/papers/source.pdf");
    assert_eq!(json[0]["bytes"], 9);
}
