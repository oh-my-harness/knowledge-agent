...
会话中可以直接要求 agent 摄入资料：

- 网页链接：在聊天中贴 URL 并要求总结、建资料卡，agent 会读取网页、总结内容、查找相关笔记，并生成 Obsidian 资料卡。
- 本地 PDF：在聊天中给出 vault 内 PDF 路径，agent 会提取 PDF 文本、总结内容、查找相关笔记，并生成资料卡。
